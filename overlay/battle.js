(() => {
  ...
  function renderPanel(panel,mon){const ratio=Math.max(0,mon.hp/mon.maxHp),fill=$("[data-fill]",panel);$("[data-name]",panel).textContent=mon.name;$("[data-level]",panel).textContent=`Lv. ${mon.level}`;$("[data-percent]",panel).textContent=`${Math.ceil(ratio*100)} %`;fill.style.width=`${ratio*100}%`;fill.classList.toggle("medium",ratio<=.5&&ratio>.2);fill.classList.toggle("low",ratio<=.2);const badge=$("[data-status]",panel);badge.hidden=!mon.status;badge.className=`status ${mon.status||""}`;badge.textContent=mon.status==="burned"?"BRN":mon.status==="paralyzed"?"PAR":"";const hp=$("[data-exact]",panel);if(hp)hp.textContent=`${mon.hp} / ${mon.maxHp} KP`;}
