(() => {
  ...
  const dom = { battle:$("#battle"),intro:$("#battleIntro"),introNames:[...document.querySelectorAll("[data-intro-name]")],introElos:[...document.querySelectorAll("[data-intro-elo]")],introTeams:[...document.querySelectorAll("[data-intro-team]")],left:$("#playerMon"),right:$("#enemyMon"),leftPanel:$("#playerPanel"),rightPanel:$("#enemyPanel"),leftTeam:$("#teamLeft"),rightTeam:$("#teamRight"),leftTrainer:$("#trainerLeft strong"),rightTrainer:$("#trainerRight strong"),message:$("#message"),turn:$("#turn"),damage:$("#damage"),effect:$("#effect"),winner:$("#winner") };
  let token = 0;
  ...
  function compactMessage(text){let m;
    if((m=text.match(/^(.+?)s (.+?) geht daneben/)))return`${m[1]} verfehlt mit ${m[2]}.`;
    if((m=text.match(/^(.+?) setzt (.+?) ein.*keine Wirkung/)))return`${m[1]} benutzt ${m[2]}. Keine Wirkung.`;
    if((m=text.match(/^(.+?) setzt (.+?) ein.*Angriff steigt/)))return`${m[1]} benutzt ${m[2]}.`;
    return text;
  }

  function eventsFrom(result){
    const state={teams:freshTeams(),active:[0,0]},events=[{type:"message",text:`${playerNames[0]} gegen ${playerNames[1]}.`}];
    for(const entry of result.log){const text=entry.text;let m;
      ...
      if((m=text.match(/^(Team Blau|Team Rot) schickt (.+?) in den Kampf/))){const side=m[1]==="Team Blau"?0:1,to=findMon(state,m[2],side);if(to){state.active[side]=to.index;events.push({type:"switch",side,to:{...to},text:`${playerNames[side]} wechselt ${to.name} ein.`});}continue;}
      if((m=text.match(/^(.+?) wird gegen (.+?) ausgewechselt/))){const from=[active(state,0),active(state,1)].find(mon=>mon.name===m[1]),to=from&&findMon(state,m[2],from.side);if(from&&to){state.active[from.side]=to.index;events.push({type:"switch",side:from.side,to:{...to},text:`${playerNames[from.side]} wechselt ${to.name} für ${from.name} ein.`});}continue;}
      ...
      if((m=text.match(/^(.+?) setzt (.+?) ein und heilt (\d+) KP/))){const mon=findMon(state,m[1]);if(mon){mon.hp=Math.min(mon.maxHp,mon.hp+Number(m[3]));events.push({type:"heal",mon:{...mon},text:`${m[1]} benutzt ${m[2]}.`});}continue;}
      if((m=text.match(/^(.+?) setzt (.+?) ein[. –]+(.+?) wird (verbrannt|paralysiert)/))){const target=findMon(state,m[3]);if(target){target.status=m[4]==="verbrannt"?"burned":"paralyzed";events.push({type:"status",target:{...target},text:`${m[1]} benutzt ${m[2]}.`});}continue;}
      if((m=text.match(/^(.+?) erleidet (\d+) Verbrennungsschaden/))){const mon=findMon(state,m[1]);if(mon){const amount=Number(m[2]);mon.hp=Math.max(0,mon.hp-amount);events.push({type:"damage",mon:{...mon},amount,text:`${mon.name} erleidet Verbrennungsschaden.`});}continue;}
      if((m=text.match(/^(.+?) ist kampfunfähig/))){const mon=findMon(state,m[1]);if(mon){mon.fainted=true;mon.hp=0;events.push({type:"faint",mon:{...mon},text:`${mon.name} wurde besiegt.`});}continue;}
      if(text.includes("geht daneben")||text.includes("keine Wirkung")||text.includes("Angriff steigt"))events.push({type:"message",text:compactMessage(text)});
    }
  ...
  function renderIntroTeam(root,team){root.replaceChildren();team.slice(0,3).forEach(mon=>{const ball=document.createElement("div"),img=document.createElement("img");ball.className="intro-ball";img.alt=mon.name||"Pokémon";installImage(img,`/sprites/icons/${mon.dexId}.png`,`https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${mon.dexId}.png`);ball.append(img);root.append(ball);});}
  async function playIntro(){dom.introNames.forEach((node,side)=>{node.textContent=playerNames[side];});dom.introElos.forEach((node,side)=>{node.textContent=`Elo ${playerElos[side]}`;});dom.introTeams.forEach((node,side)=>renderIntroTeam(node,scenario[side?"teamB":"teamA"]));dom.intro.classList.add("show");await sleep(3000);dom.intro.classList.remove("show");await sleep(300);}
  function renderPanel(panel,mon){const ratio=Math.max(0,mon.hp/mon.maxHp),fill=$("[data-fill]",panel);$("[data-name]",panel).textContent=mon.name;$("[data-level]",panel).textContent=`Lv. ${mon.level}`;fill.style.width=`${ratio*100}%`;fill.classList.toggle("medium",ratio<=.5&&ratio>.2);fill.classList.toggle("low",ratio<=.2);const badge=$("[data-status]",panel);badge.hidden=!mon.status;badge.className=`status ${mon.status||""}`;badge.textContent=mon.status==="burned"?"BRN":mon.status==="paralyzed"?"PAR":"";}
  ...
  function clearBattleView(){
    ...
    dom.intro.classList.remove("show");
  ...
  async function loadRankedBattle(){try{const response=await fetch("/api/ranked-battle",{cache:"no-store"});if(!response.ok)return false;const battle=await response.json();if(!battle?.id||!battle?.simulation)return false;currentBattleId=battle.id;scenario={seed:battle.seed,teamA:battle.battleTeams?.[0]||scenario.teamA,teamB:battle.battleTeams?.[1]||scenario.teamB};playerNames=[battle.players?.[0]?.display||"Team Blau",battle.players?.[1]?.display||"Team Rot"];playerElos=battle.players?.map(player=>Number.isFinite(Number(player?.elo))?Math.round(Number(player.elo)):1000)||[1000,1000];dom.leftTrainer.textContent=playerNames[0];dom.rightTrainer.textContent=playerNames[1];serverResult=battle.simulation;return true;}catch(error){console.warn("Ranked-Kampf konnte nicht geladen werden",error);return false;}}
  async function completeRankedBattle(){if(!currentBattleId)return false;for(let attempt=1;attempt<=5;attempt+=1){try{const response=await fetch(`/api/ranked-battle/${encodeURIComponent(currentBattleId)}/complete`,{method:"POST",headers:{"Content-Type":"application/json"},body:"{}",cache:"no-store"});if(response.ok)return true;}catch(error){console.warn("Kampfabschluss konnte nicht bestätigt werden",error);}await sleep(500*attempt);}return false;}
  async function run(){const found=await loadRankedBattle();if(!found){clearBattleView();setTimeout(run,2000);return;}const runToken=++token,result=serverResult,events=eventsFrom(result),state={teams:freshTeams(),active:[0,0]};dom.battle.style.visibility="visible";for(const node of [dom.left,dom.right,dom.leftPanel,dom.rightPanel]) node.style.opacity="";dom.winner.hidden=true;dom.turn.textContent="KAMPFBEGINN";renderActive(state,0,false);renderActive(state,1,false);await playIntro();if(runToken!==token)return;renderActive(state,0,true);renderActive(state,1,true);await sleep(700);
    for(const event of events){if(runToken!==token)return;
      ...
      else if(event.type==="damage"){const mon=findMon(state,event.mon.name);mon.hp=event.mon.hp;dom.message.textContent=event.text;renderPanel(mon.side?dom.rightPanel:dom.leftPanel,mon,!mon.side);await sleep(800);}
    ...
    if(runToken===token){const completed=await completeRankedBattle();if(completed){currentBattleId=null;serverResult=null;clearBattleView();setTimeout(run,500);}else{setTimeout(run,2000);}}
  }
