...
const { RANKED_QUEUE_JSON } = require("./paths");
...
function isRankedLocked(userId, caughtAt) {
  const queue = rankedQueue.readQueue(RANKED_QUEUE_JSON, readJson);
  return rankedQueue.lockedCaughtAt(queue, String(userId), Number(caughtAt));
}
...
function handleTrade({ userId, userName, rawInput }) {
  ...
  if (isRankedLocked(fromId, offerP.caughtAt) || isRankedLocked(toId, wantP.caughtAt)) {
    writeText(outFile, "❌ Eines der Pokémon ist derzeit durch die Ranked-Suche gesperrt.");
    return { ok: false, reason: "ranked_pokemon_locked" };
  }
...
function createWidgetTrade({ fromId, fromDisplay, toId, offerCaughtAt, wantCaughtAt }) {
  ensureDirs();
  fromId = String(fromId || "").trim();
  toId = String(toId || "").trim();
  if (!fromId || !toId || fromId === toId) return { ok: false, reason: "invalid_target" };

  const pokedex = migratePokedexIfNeeded(readJson(POKEDEX_JSON, { users: {} }));
  const fromEntry = pokedex.users?.[fromId];
  const toEntry = pokedex.users?.[toId];
  if (!fromEntry?.caught?.length || !toEntry?.caught?.length) return { ok: false, reason: "missing_inventory" };

  const offerP = fromEntry.caught.find((mon) => Number(mon?.caughtAt) === Number(offerCaughtAt));
  const wantP = toEntry.caught.find((mon) => Number(mon?.caughtAt) === Number(wantCaughtAt));
  if (!offerP || !wantP) return { ok: false, reason: "missing_pokemon" };
  if (isRankedLocked(fromId, offerCaughtAt) || isRankedLocked(toId, wantCaughtAt)) return { ok:false, reason:"ranked_pokemon_locked" };

  let trades = cleanupTrades(readTrades());
  const duplicate = Object.values(trades.pending || {}).some((entry) =>
    String(entry?.fromId) === fromId && String(entry?.toId) === toId &&
    Number(entry?.offer?.caughtAt) === Number(offerCaughtAt) &&
    Number(entry?.want?.caughtAt) === Number(wantCaughtAt)
  );
  if (duplicate) return { ok: false, reason: "duplicate_trade" };

  const id = String(trades.nextId++);
  const resolvedFromDisplay = String(fromDisplay || fromEntry.display || "Trainer");
  const resolvedToDisplay = String(toEntry.display || "Trainer");
  trades.pending[id] = {
    id,
    fromId,
    fromDisplay: resolvedFromDisplay,
    toId,
    toDisplay: resolvedToDisplay,
    offer: offerP,
    want: wantP,
    createdAt: now(),
  ...
  writeTrades(trades);
  writeText(
    tradeMessageFile(fromId),
    `🧾 Trade #${id}: @${resolvedFromDisplay} bietet ${describePokemon(offerP)} für @${resolvedToDisplay}s ${describePokemon(wantP)}.`
  );
...
function handleAccept({ userId, userName, rawInput }) {
  ...
  if (isRankedLocked(tr.fromId, tr.offer?.caughtAt) || isRankedLocked(toId, tr.want?.caughtAt)) {
    writeText(outFile, `❌ Trade #${tradeId} ist während einer Ranked-Suche gesperrt.`);
    return { ok:false, reason:"ranked_pokemon_locked" };
  }
  ...
  const offerBefore = JSON.parse(JSON.stringify(offerPokemon));
  const wantBefore = JSON.parse(JSON.stringify(wantPokemon));
  ...
  try {
    ...
    profiles.users[tr.fromId] ??= { display: tr.fromDisplay, party: { activeSlot: 0, slots: Array(6).fill(null) }, progress: {} };
    profiles.users[toId] ??= { display: toDisplay, party: { activeSlot: 0, slots: Array(6).fill(null) }, progress: {} };
    const historyEntry = {
        id: String(tradeId),
        completedAt: now(),
        players: [
          { id: tr.fromId, display: tr.fromDisplay, gave: offerBefore, received: wantPokemon },
          { id: toId, display: toDisplay, gave: wantBefore, received: offerPokemon },
        ],
    };
    profiles.users[tr.fromId].tradeHistory ??= [];
    profiles.users[toId].tradeHistory ??= [];
    profiles.users[tr.fromId].tradeHistory.unshift(historyEntry);
    profiles.users[toId].tradeHistory.unshift(historyEntry);
    profiles.users[tr.fromId].tradeHistory = profiles.users[tr.fromId].tradeHistory.slice(0, 100);
    profiles.users[toId].tradeHistory = profiles.users[toId].tradeHistory.slice(0, 100);
