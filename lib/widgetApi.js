...
function installWidgetApi(app, { paths, readJsonSafe, setNoCache }) {
  app.get("/api/widget/meta", (req, res) => {
    ...
    res.json({ ok:true, ranks:{ catches:rank("catches"),distinct:rank("distinct"),battles:rank("battles") }, ranked:ranked.publicRank(profiles,userId), availablePlayers:Object.values(presence.users || {}).filter((entry)=>String(entry?.id)!==userId && Date.now()-Number(entry?.lastSeenAt||0)<=30*60*1000) });
  });
