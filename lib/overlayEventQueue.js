...
function spawnNeedsStart(event, spawnState) {
  if (!event || event.type !== "spawn" || event.status !== "active") return false;
  return !(spawnState?.active && spawnState?.pokemon);
}
...
module.exports = { emptyState, read, write, enqueue, head, claimHead, complete, hasOpenType, spawnNeedsStart, clear };
