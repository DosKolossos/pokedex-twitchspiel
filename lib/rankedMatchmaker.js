...
function pairOldest(queue, joiningUserId) {
  ...
  const opponent = Object.values(queue.entries || {})
    .filter((entry) => String(entry.userId) !== String(joiningUserId))
...
function startIfReady({ queue, joiningUserId, profiles, now = Date.now() }) {
  ...
  if (!pair) return null;
  const [a, b] = pair;
  delete queue.entries[String(a.userId)];
  delete queue.entries[String(b.userId)];
  ...
  profiles.users ??= {};
  const winnerProfile = profiles.users[String(winner.userId)];
  const loserProfile = profiles.users[String(loser.userId)];
...
module.exports = { pairOldest, battleId, startIfReady };
