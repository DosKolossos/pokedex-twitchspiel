...
function handleCatch({ userId, userName }) {
  ...
  writeText(CATCH_MESSAGE_TXT, `✅ @${userName} ist beim Fangversuch dabei! (Teilnehmer: ${state.participants.length})`);
  return { ok: true, participants: state.participants.length };
}
