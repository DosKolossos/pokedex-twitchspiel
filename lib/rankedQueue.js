...
function clearQueue(filePath, readJsonSafe) {
  const queue = readQueue(filePath, readJsonSafe);
  const removed = Object.keys(queue.entries).length;
  writeQueue(filePath, emptyQueue());
...
function toggleFromChat({ filePath, readJsonSafe, profile, userId, userName, rawInput, now = Date.now() }) {
  ...
  // Depending on the caller, rawInput is either the complete chat message
  // ("!ranked stop"), the command without prefix ("ranked stop"), or only
  // the argument ("stop" / ""). Normalize all supported forms here.
  const normalizedInput = String(rawInput || "")
    .normalize("NFKC")
    .replace(/[\u200B-\u200D\u2060\uFEFF]/g, "")
    .trim()
    .toLowerCase();
  const argument = normalizedInput
    .replace(/^!?ranked\b/i, "")
    .trim();
  ...
  if (isQueued) {
    if (queue.entries[id].status === "battling") {
      return { ok: false, reason: "battle_in_progress", message: `@${display} Dein Ranked-Kampf läuft bereits.` };
    }
  ...
  const configuredCooldown = Number(process.env.RANKED_COMMAND_COOLDOWN_SECONDS ?? 10);
  const cooldownMs = Math.max(0, Number.isFinite(configuredCooldown) ? configuredCooldown : 10) * 1000;
  ...
  queue.entries[id] = { userId: id, display: profile.display || display, joinedAt: now, status:"waiting", team: snapshot };
  writeQueue(filePath, queue);
  return {
    ok: true,
    action: "join",
...
module.exports = { readQueue, writeQueue, clearQueue, publicEntry, lockedCaughtAt, snapshotTeam, toggleFromChat, resetCommandCooldownsForTests };
